...
namespace gandiva {
  ...
  class GANDIVA_EXPORT SplitHolder: public FunctionHolder {
  public:
  ~SplitHolder() override = default;

  // Invoked by function_holder_registry.h
  static Status Make(const FunctionNode &node, std::shared_ptr<SplitHolder> *holder);

  static Status Make(const std::string& sql_pattern, std::shared_ptr<SplitHolder>* holder);
  ...
  const char *operator()(ExecutionContext* ctx, const char* user_input,
                         int32_t user_input_len, const char* pattern, int32_t *out_length) {
  ...
  explicit SplitHolder(const std::string& pattern): pattern_(pattern), re2_(pattern) {}
