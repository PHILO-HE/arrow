...
namespace gandiva {
  ...
  Status SplitHolder::Make(const FunctionNode& node, std::shared_ptr<SplitHolder>* holder) {
    ...
    ARROW_RETURN_IF(
        ...
        Status::Invalid("'split part' function requires a literal as the second parameter"));
  ...
  Status SplitHolder::Make(const std::string& sql_pattern,
                             std::shared_ptr<SplitHolder>* holder) {
    auto splitHolder = std::shared_ptr<SplitHolder>(new SplitHolder(sql_pattern));
    ARROW_RETURN_IF(!splitHolder->re2_.ok(),
                    Status::Invalid("Building RE2 pattern '", sql_pattern, "' failed"));
    *holder = splitHolder;
    return Status::OK();
  }

  void SplitHolder::return_error(ExecutionContext* context,
